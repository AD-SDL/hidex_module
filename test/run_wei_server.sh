...
tmux send-keys -t $session:$window 'cd ' $folder C-m
tmux send-keys -t $session:$window 'python3 -m wei.server --workcell ./test_hidex_workcell.yaml' C-m
...
tmux send-keys -t $session:$window 'python3 -m wei.engine --workcell ./test_hidex_workcell.yaml' C-m
